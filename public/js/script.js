function initializeLegacyScripts() {
    if (window.__legacyScriptsInitialized) {
        return;
    }
    window.__legacyScriptsInitialized = true;
    var thatPD;
    ...
    class TextRunningEffect {
        ...
        windowScroll() {
            var section = $('.text-running-way-sec').first();
            if (!section.length) {
                return;
            }
            var forward;
            ...
            if ($(window).scrollTop() + window.innerHeight >= section.offset().top && $(window).scrollTop() <= section.offset().top + section.innerHeight()) {
                if (thatTR.flag) {
                    thatTR.flag = false;
                    if (forward) {
    ...
    function updateBoardTransforms(stickyElement, boardSelector) {
        var boards = $(boardSelector);
        if (!stickyElement.length || boards.length === 0) {
            return;
        }
        ...
        var blockHeight = window.innerHeight * 1.5;
        var moveDistance = window.innerHeight;
        if (scrollTop <= stickyTop) {
            boards.css("transform", "translate(0px, 0px)");
            return;
        }
        boards.each(function (index, element) {
            var start = stickyTop + blockHeight * index;
            var end = start + blockHeight;
            var translateY = 0;
            if (scrollTop <= start) {
                translateY = 0;
                ...
                var progress = (scrollTop - start) / blockHeight;
                translateY = -(progress * moveDistance);
            }
            $(element).css("transform", "translate(0px, " + translateY + "px)");
        });
    }
    ...
    class ThreeBoardEffect {
        ...
        init() {
            ...
            $(window).resize(that3.windowScroll);
            that3.windowScroll();
        }

        windowScroll() {
            if (!that3.stickyHeight.length) {
                return;
            }
            updateBoardTransforms(that3.stickyHeight, ".single-board");
        }
    }
    ...
    class PublicationEffect {
      constructor() {
        ...
        thatPub.scrollPricing = $(".pricing-section").first();
        thatPub.publicationList = $(".horizontal-scroll-container-2 .all-publication").first();
        if (!thatPub.scrollContainer.length || !thatPub.publicationList.length) {
            thatPub = null;
            return;
        }
        thatPub.init();
      ...
      windowScroll() {
        if (!thatPub || !thatPub.scrollContainer.length) {
            return;
        }
        var containerTop = thatPub.scrollContainer.offset().top;
        var containerBottom = containerTop + thatPub.scrollContainer.outerHeight(true);
      ...
      windowResize() {
        ...
        if (window.innerWidth > 600) {
          thatPub.portfolioWidth = thatPub.publicationList.outerWidth(true);
          ...
          thatPub.portfolioWidth =
            thatPub.publicationList.outerWidth(true) + 300;
    ...
    if ($(".horizontal-scroll-container-2").length) {
...
function triggerLegacyScripts() {
    ...
    if (document.querySelector('.landing-section')) {
        ...
        setTimeout(triggerLegacyScripts, 50);
    }
}

window.addEventListener('react-app-ready', triggerLegacyScripts);
window.addEventListener('load', triggerLegacyScripts);
triggerLegacyScripts();
